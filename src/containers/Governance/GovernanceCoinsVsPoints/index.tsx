...
const GovernanceCoinsVsPoints: FC = () => {
  return (
    <DocContainer className="GovernanceCoinsVsPoints" title="Coins vs. Points" lastUpdated="07 Mar 2021">
      ...
      <p>
        Blockchain networks typically rely on transaction fees to both incentivize server owners and also prevent spam.
        While fees do solve the spam problem by making it expensive to attack or spam the network, fees also introduce a
        new problem. Imagine needing to pay a fee every time you send an email or change your password. When users are
        nickel and dimed for every interaction they perform they will stop using those apps. For a blockchain network
        that would like to support app development, prevent spam, and incentivize server owners, another solution is
        needed.
      </p>

      <p>
        The goal of the point system is to address these challenges. By locking up coins, users receive points (which
        fill back up over time) in return. The specifics of how points solve each challenge are as follows:
      </p>
