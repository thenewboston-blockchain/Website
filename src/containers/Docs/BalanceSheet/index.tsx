  ...
  return (
    <section className="BalanceSheet">
      ...
      <p>
        ...
        notebook. In doing so, his copy of the balance sheet acted as a "checkpoint" in which earlier transaction
      ...
      <p>
        This is why in the real network, it is important for all nodes to maintain a copy of the transaction history for
        as far back as they are able to. In practice, nodes will never need to maintain a record of transactions more
        than a few hours old. This is because every node in the system is always in constant communication with each
        other, and every node is always verifying the transactions of others. Even though the architecture is structured
        around a central validator node, if the transaction ledger produced by that validator node is ever deemed
        "invalid" by the bank nodes who are continuously verifying the data, the banks will quickly lose trust in the
        validator, reject the updated transaction ledger and balance sheet, and find a new validator as a replacement.
      </p>
      <p>
      ...
      <ul>
        ...
        <li>
          the further back in time a node is able to provide a historical transaction log, the more transactions you are
          able to verify (by comparing them against your own) and the more trust can be established between nodes
        </li>
      </ul>
    </section>
  );
