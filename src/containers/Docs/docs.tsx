...
const chapter03 = (
  <section>
    ...
    <p>
      ...
      several accounts (key pairs) through the same account manager application. This can be thought of as an individual
...
const chapter05 = (
  <section>
    ...
    <p>
      ...
      transactions fees to 0 to allow for free usage of their nodes. It is recommended however for nodes to set a small
      fee to deter malicious users from attacking the network and bloating the transaction log by sending a large amount
      of useless transactions between accounts.
    </p>
    <p>
    ...
    <p>
      Users will always attempt to join banks that offer the lowest transaction fees. There are also additional factors
      such as server reliability and other services the bank may provide, but the transaction fees will likely be a
      large consideration. When bank fees become too high, the members will look for a new bank to join.
    </p>
    <p>
      ...
      displayed in the user interface. When creating a transaction, Amy will first choose the desired recipient and then
      the amount of points she wishes to send. A very simple UI that Amy may see at that point might look like this:
    </p>
    <table>
      <tr>
        <td>
    ...
    <p>
      ...
      <ol>
        ...
        <li>
          After the transaction is confirmed by the validator, the validator will add that transaction to the ledger and
          update the balance sheet for both Amy and Brian.
        </li>
        <li>
    ...
    <p>
      ...
      transactions only allow for the transfer of points from one account to another. Since the bank and validator also
      receive points, the entire set of transactions involved would more closely resemble this:
    </p>
    <table>
      <thead>
        <tr>
...
export const getChapterContent = (chapter: number) => {
  switch (chapter) {
    case 1:
      return chapter01;
    case 2:
